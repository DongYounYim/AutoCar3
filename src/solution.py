...
def unexpected_drive():
    ...
    # TODO: 이 아래부터 skeleton 코드
    if detect == "plastic_bag":
        current_speed = 20
        drv.setSpeed(current_speed)
        drv.forward()
        time.sleep(3)
        ...
        for _ in range(3):
            drv.setLamp(1, 1)
            drv.alarm(scale=4, pitch=10, duration=1)
            time.sleep(1)
            drv.setLamp(0, 0)
        drv.forward()
    else:
...
def ramp_drive():
    ...
    # TODO: 이 아래부터 skeleton 코드
    drv.setSensorStatus(euler=1)
    while True:
        frame = cam.value
        slope = drv.getEuler('pitch')
        intSlope = int(slope)
        drv.forward(speed-intSlope)
        
        # 신호등 탐지 진행
        # 만약 내리막에서 감지가 될 정도이면 순차적 로직으로 구현
...
def obstacle_drive():
    ...
    while True:
        ...
        right = []
        for v in vectors:
            angle, distance, trust = v
            if angle > 0 and angle < 90 and int(trust) == 47:
                right.append(distance)
            if angle > 270 and angle < 360 and int(trust) == 47:
                left.append(distance)

        left = 0 if len(left) == 0 else np.mean(left) 
        right = 0 if len(right) == 0 else np.mean(right)

        if abs(left - right) < 200:
            # 직진
            drv.steering = 0
        elif left < right:
            # 우회전
...
def turn_drive():
    ...
    # 이 아래는 모두 스켈레톤 입니다.
    if detect == "right_sign":
        drv.steering = 1
        drv.forward(30)
    elif detect == "left_sign":
        drv.steering = -1
        drv.forward(30)
    else:
        drv.steering = 0
